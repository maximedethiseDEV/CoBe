...
import {Address, Company, Contact, SharedDetails, TransportSupplier} from '@core/models';
    ...
    forkJoin([contact$, shared$])
      .pipe(
        switchMap(([contact, shared]) => {
          ...
          return this.transportSupplierProvider.create(payload as TransportSupplier);
        })
      )
      .subscribe({
        next: () => {
          this.messageService.add({
            severity: 'success',
            summary: 'Enregistré',
            detail: 'Transporteur enregistré',
            life: 2000
          });
          this.back();
        },
